...
def addDownload(name, url, image, provider=None):
    ...
    from resources.lib.indexers import huf
    url = huf.resolver().link(url)
    if url == None: return
    ...
    try:
        ...
        no = control.yesnoDialog(dest, 'Complete file is' + size, 'Continue with download?', name + ' - ' + 'Approve Selected Item?', 'Approve', 'Cancel')
