...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'https://pastebin.com/raw/Gyathm8i'
            self.list = self.entire_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.entire_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.entire_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            link = 'http://iamchief.offshorepastebin.com/search.xml'
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.entire_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['HUF']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def entire_list(self, url, result=None):
        try:
        ...
        for item in items:
            try:
                ...
                if re.search(r'\d+', date): name += ' [COLOR dodgerblue] Updated %s[/COLOR]' % date
